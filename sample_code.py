...
def run_single_benchmark(
    dataset_path, mdl_name, sys_prompt, temperature=1, log_path="outputs/"
):
    """
    # Description
...
"""
    ...
    for i in range(1, 6):
        predicted_result = run_single_benchmark(
    ...
    df_results.to_csv(
        f"{log_folder}/predition_{mdl_name}_{prompt_name}.csv", index=False
    )

    print(
        f"Prediction results saved to {log_folder}/predition_{mdl_name}_{prompt_name}.csv"
    )
